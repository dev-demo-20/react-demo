...
export const checkSelectorExist = (selector: ElementFinder) =>
  selector !== undefined;
 ...
 */
export const waitUntilClickable = async (
  selector: ElementFinder,
  classname = '',
  timeout = waitUntilDisplayedTimeout
) => {
...
export const waitForCount = (
  ...
  return elementArrayFinder
    .count()
    .then((actualCount) => expectedCount === actualCount);
};
...
export const getModifiedDateSortButton = (): ElementFinder =>
  element(by.id('modified-date-sort'));
...
export const getToastByInnerText = (text: string): ElementFinder =>
  element(by.css('.Toastify__toast-container')).element(
    by.cssContainingText('div[role=alert]', text)
  );
 ...
 * be rejected.
 */
export const waitUntilAnyDisplayed = async (
  selectors: ElementFinder[],
  timeout = waitUntilDisplayedTimeout
): Promise<void> => {
  await browser.wait(
    ExpectedConditions.or(
      ...selectors.map((selector) => ExpectedConditions.visibilityOf(selector))
    ),
    timeout,
...
//FUNCTION: TO CLEAR AND SET A VALUE IN TEXTFIELD
export const clearAndSetTextfield = (element, data): any => {
  clearText(element, 100);
  element.sendKeys(data);
};

//FUNCTION: TO CUSTOM CLEAR TEXTFIELD
//Date and some other fields dont work with clear() function -- Added backspace functionality
export const clearText = (elem: any, length: number = 100) => {
  elem.clear();
  length = length || 100;
  var backspaceSeries = '';
